...
class TenantDashboard():
    ...
    def SearchAdsManager(self,search):
        print('*' * 50)
        sleep(10)
        searchAdsManSearchPlaceholder = driver.find_element(By.ID, "inp-base-searchbox-new").get_attribute('placeholder')
        searchBox=driver.find_element(By.ID, "inp-base-searchbox-new")
        if searchBox.tag_name == 'input' and searchAdsManSearchPlaceholder == 'Search Ads Manager':
            print('search Ads manager is of Input type and placeholder is correct and test passed!!')
            searchBox.clear() 
            searchBox.send_keys(search,Keys.ENTER)
            sleep(4)
        else:
            print('search Ads manager is not of Input type.')
        allButtons=driver.find_elements(By.XPATH, "//div[@class='result-types']/button")
        print('Total buttons present are:',len(allButtons))
        buttons = ["tenants", "org", "accounts", "campaigns", "adgroup", "creatives"]
        for i in range(len(buttons)):           
            button = driver.find_element(By.XPATH, "//button[contains(@id, 'btn-search-"+buttons[i]+"')]")   
            if button.is_selected() == False:
                print('clicked on button:',buttons[i])
                button.click()
                # print(button[i])
                sleep(4)
            else:
